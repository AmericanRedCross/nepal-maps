...
function callModal (thumbnail) {
    ...
    var previewImgSrc = "../img/thumbs/" + $(thumbnail).attr("data-filename").slice(0,-4)+'_thumb.jpg'
          ...
          item.extent+' '+item.sector+'" data-filename="'+ item.filename +'">' +
            '<div class="row thumbnail" style="min-height:0; margin-left:0; margin-right:0; padding:10px">'+
                ...
                '<div class="col-sm-4">'+
                    ...
                    '</div>' +
                    '<button type="button" onclick="callModal(' + item.thumbnail_id + ');" class="btn btn-success btn-mini">Preview Thumbnail</button>' +
                '</div></div></div>';
        html=html+itemhtml;
